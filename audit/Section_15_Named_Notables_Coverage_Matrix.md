...
| `해양` |  | `미다스`, `해양 실비아`, `이소벨`, `마르코 바르텔로`, `엘레오노라 라 크루즈`, `골드핑거 바스`, `리나 웨이브서프`, `에릭 시스트롬`, `오렌`, `세일블레스 마리아`, `모로스`, `크리스토퍼 델마르` | `해양 실비아 name collision`, `대부분 A급/히어로급/제독/단장 신호`, `top five + city-role + tail unnamed pass 완료`, `심연 장부관은 이소벨 adjacency only`, `망자항해 기록관은 모로스 merge 금지 메모` | `thin with many boundary signals and strong place-institution slots; oceanic unnamed slot pass complete with Christopher Delmar boundary hold retained` |
