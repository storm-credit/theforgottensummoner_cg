...
1. `프로스트`: `원로 사냥꾼`, `묘지기 장로`, `대예언자`, `수석 기술자 / 드워프 장인`, `별의 샤먼` read-only narrowing
...
현재 `프로스트`가 다음 1순위인 이유:

- `오벨리스크`는 핵심 slot narrowing이 2차까지 닫혀 ROI가 낮아졌다.
- `프로스트`는 risk가 크지만, `오로라 평원`, `얼음무덤 언덕`, `푸른 폭풍 요새`, `겨울회의 의장막` 앵커가 선명해 place-first pass로 자를 수 있다.
- `원로 사냥꾼`, `묘지기 장로`, `대예언자`, `수석 기술자`, `별의 샤먼`은 다음 한 배치로 묶기 좋다.
