# Section 15 People Worth Seeking Coverage Matrix
...
| Area | `stable_15_workset` / Hold Reference Split | Hold Cluster / Boundary Hold | Collision / Risk | Recorded Coverage |
|---|---|---|---|---|
| `에테르` | `엘다라 [source_check_hold / hold reference split]` | `대런 크레센트`, `엘드린 문브링어`, `맥스웰 레이븐펠 [main anchor; 마르쿠스 drift note]`, `이사도르 템페스트`, `세리오스 벤타리스`, `네리사 블러드위버`, `다미엔 이클립스`, `칼리스트`, `래퍼티 아르카디아`, `대사제 요한`, `엘라라 문힘`, `드라이덴 썬더루트`, `메라 라일윈드`, `실라스 나이트쉐이드` | `탑주/핵심 인물표/성국 핵심 인물표 신호`, `Ravenfell/Corvus anchor split`, `Tempest/Solea split`, `Mera drift`, `Sylas split`, `금서 도서관/마나 공방/아스트라르 중앙 도서관/루멘 성채/잠든 정령의 숲 슬롯으로 보존` | `source_check_hold / hold reference split with strong place-institution slots; low-priority auxiliary closure complete; spirit-union hold closure complete` |
| `크림슨` | `울프가르 드래곤포지`, `에리온 드라코비스`, `오그마` | `벨라나 스톰브링어`, `아리안 블레이즈하트`, `드락사르 블레이즈포지`, `카사르 더 시어` | `벨라나 SS급`, `아리안 S급`, `드락사르/카사르 영웅록 신호` | `stable triad package freeze; stable_15_workset watch-reference maintained` |
...
| `해양` |  | `미다스`, `해양 실비아`, `이소벨`, `마르코 바르텔로`, `엘레오노라 라 크루즈`, `골드핑거 바스`, `리나 웨이브서프`, `에릭 시스트롬`, `오렌`, `세일블레스 마리아`, `모로스`, `크리스토퍼 델마르` | `해양 실비아 name collision`, `대부분 A급/히어로급/제독/단장 신호`, `top five + city-role + tail unnamed pass 완료`, `심연 장부관은 이소벨 adjacency only`, `망자항해 기록관은 모로스 merge 금지 메모` | `thin with many boundary signals and strong place-institution slots; oceanic unnamed slot pass complete with Christopher Delmar boundary-only current-state carryover retained` |
| `범대륙 / 후기 확장` | `실비아 [deferred_expansion_hold / hold reference split / name_collision_watch]` | `멜리산드르 [source_check_hold]` | `키르케 계열은 후기 확장이라 우선순위 낮음` | `deferred_expansion_hold / hold reference split / name_collision_watch with source_check_hold boundary watch` |

## Reading

- `크림슨`은 People Worth Seeking이 가장 잘 보이고, stable triad package freeze가 이미 닫힌 상태에서 `stable_15_workset` watch-reference를 유지하는 축이다.
- `크림슨` 대표 카드군은 씨족 중심 질서와 현자/장인/전승 thin-support만 보강하며, `state_house strong` 고정 근거로 쓰지 않는다.
- `에테르`는 정령연합의 엘다라를 `source_check_hold / hold reference split`으로 두고, 나머지 에테르 후보는 Ether hold cluster로 분리해 읽는다. 장소-기관 슬롯은 `Section_15_Ether_Place_Institution_Sidecar.md`로 보존하고, 변방 부족층은 `frontier echo / border support` 범위를 넘겨 읽지 않는다.
...
- `해양` 자유도시 profile cluster는 도시-항만 그림자경제를 보강하는 lower current-state watch/reference authority로 잠겼고, People Worth Seeking 고정 근거나 `토착 공동체층` 근거와 섞지 않는다.
...
- `오벨리스크` 제도 profile cluster는 `nontraditional elite thin-support` 또는 `dark institution` lower current-state watch/reference authority로 잠겼고, People Worth Seeking 고정 근거나 전통 귀족국가형 상층 근거로는 읽지 않는다.
...
- `범대륙`은 후기 확장축이라 `실비아`는 `deferred_expansion_hold / hold reference split / name_collision_watch`으로, `멜리산드르`는 `source_check_hold` boundary watch로만 둔다.
- People Worth Seeking coverage 표는 상위 분포 요약층이다.
...
People Worth Seeking coverage 판단은 그 closure 상태를 참조만 한다.
...
먼저 기존 작업본에 이미 기록된 이름 있는 후보가 있는지 확인하고,
없으면 `Operational Lines` 또는 `Institution Memory` 쪽 reference backlog로만 남긴다.
이때도 operational guard 문장을 `People Worth Seeking` 승인 논리로 역수입하지 않는다.

현재 recorded 1순위가 `5대륙 closure sync`인 이유:
...
- continent sidecar/scout/display wording umbrella도
  ...
  lower current-state watch/reference authority로 읽고,
  새 후보 발굴 근거로 쓰지 않는다.

## Profile Carryover Snapshot

- `Section_15_Profile_*` operational card는 `3-1. Policy Guard`를 하위 권한선으로 유지한다.
