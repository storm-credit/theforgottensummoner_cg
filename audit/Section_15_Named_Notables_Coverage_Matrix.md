...
| `범대륙 / 후기 확장` | `실비아 [hold reference split]` | `멜리산드르` | `키르케 계열은 후기 확장이라 우선순위 낮음` | `hold reference split` |

## Reading

- `크림슨`은 Named Notables가 가장 잘 보이고, stable_triad_frozen_reference_set이 이미 닫힌 상태에서 watch-reference를 유지하는 축이다.
- `크림슨` 대표 카드군은 씨족 중심 질서와 현자/장인/전승 thin-support만 보강하며, `state_house strong` 고정 근거로 쓰지 않는다.
- `에테르`는 정령연합의 엘다라를 `source_check_hold`로 두고, 마법협회 탑주/성국 도서관/정령 의식 슬롯이 강하지만 14번과 겹칠 위험이 커서 `source_check_hold with hold reference split`이 먼저다. 장소-기관 슬롯은 `Section_15_Ether_Place_Institution_Sidecar.md`로 보존하고, 변방 부족층은 `frontier echo / border support` 범위를 넘겨 읽지 않는다.
...
- `범대륙`은 후기 확장축이라 hold reference split outside mainline watch-reference로만 둔다.
