...
우선순위 `P1` 후보 중
이번 mixed_keep 재검토 사이클과 직접 연결된 항목을 다시 읽은 시트다.
...
`legacy_mixed -> section_style_reclassify`로 내려갔다.
...
현재는 이 P1 리스크 묶음을 닫힌 watch-reference로 유지하며,
`용의 후예`, `붉은 사막 부족 연합`, `바다의 교단`, `망자의 왕국`, `봉인 수호단`
중 새 local drift가 생길 때만
`P1` 리스크 밀도 기준으로 재점검한다.
