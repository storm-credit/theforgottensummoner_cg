...
| `에테르` | `엘다라` | `대런`, `엘드린`, `마르쿠스/맥스웰`, `이사도르`, `세리오스`, `네리사`, `다미엔`, `칼리스트`, `래퍼티`, `요한`, `엘라라`, `드라이덴`, `메라`, `실라스 나이트쉐이드` | 금서/공방/관측/대서고/성채/성검/서약/그늘항로/정령묘 슬롯 | `closed_for_now` |
| `프로스트` | 없음 | `울프릭`, `시그리드`, `마리안`, `프리야`, `카이라` | 원로 사냥꾼, 묘지기 장로, 대예언자, 수석 기술자, 별의 샤먼, 아이스포지 장인 | `closed_for_now` |
| `해양` | 없음 | `미다스`, `해양 실비아`, `이소벨`, `마르코`, `엘레오노라`, `골드핑거`, `리나`, `에릭`, `오렌`, `마리아`, `모로스`, `크리스토퍼 델마르` | 오라클, 항로/해도, 조선소, 감정사, 금고, 경매장, 은행, 세관, 유령선 기록 슬롯 | `closed_for_now` |
| `오벨리스크` | 없음 | `바리온`, `아이기스`, `카론`, `베스`, `이안`, `카트린`, `레보니아`, `우로스`, `세르반`, `레티시아` | 기록 수호자, 관측대장, 신성 기록소 사제, 기억 지기, 기억 경매장 중개자, 사후 서기관, 묘역 감독관 | `closed_for_now` |
...
다음 큰 흐름은 15번을 더 파기보다
`8번 세력 아카이브`와 15번 앵커를 다시 연결하는 것이다.

이유:

- 15번 후보들은 대부분 대륙/세력/도시 앵커 없이는 안정적으로 배치할 수 없다.
- 14/15 경계 판단도 결국 8번 세력 spine과 연결되어야 한다.
- 유명 NPC형 인물은 직업별이 아니라 `어느 대륙의 어느 세력/도시/조직인가`로 폴더링해야 한다.
