...
다음 큰 흐름은 `프로스트`를 다음 메인 본선으로 실제 올리는 것이다.
...
- 따라서 오벨리스크 핵심 slot read-only narrowing은 2차까지 한 번 닫혔고, 다음은 `프로스트`를 mainline으로 올리는 편이 효율이 높다.
- `프로스트`는 원로단/대예언자/장로 신호가 강하지만, `오로라 평원 / 얼음무덤 언덕 / 푸른 폭풍 요새 / 겨울회의 의장막` 앵커 기준의 place-first role slot pass로 오염을 통제할 수 있다.
- 다음 batch는 `원로 사냥꾼`, `묘지기 장로`, `대예언자`, `수석 기술자 / 드워프 장인`, `별의 샤먼` read-only narrowing이다.
