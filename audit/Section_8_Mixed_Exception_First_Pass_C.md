# Section 8 Mixed Exception First Pass C
...
- 이유:
  ...
  - 장소 밀도는 강하지만, 이는 `place_style`이 아니라 `section_style 안의 강한 location density`로 기록하는 편이 정확하다.
  - 따라서 `mixed`를 유지하기보다
    `section_style`로 내리고,
...
|---|---|---|---|---|
| `잊힌 자들의 연합 (Forgotten Alliance)` | `mixed` | `section_style_reclassify` | `nontraditional_elite_as_state_house` | 구조는 section형으로 내리되 내용 해석은 오벨리스크 생존 연합 축으로 유지 |
...
2. 구조 라벨만 `mixed -> section_style`로 내린다

따라서 앞으로는
`가문 폴더가 있으니 귀족국가형이다`도 아니고,
`장소가 강하니 mixed를 유지해야 한다`도 아니다.
