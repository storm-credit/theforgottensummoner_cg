...
1. `canonical_root`와 `quarantine_root`의 중복 범위 실측 readiness를 유지한다
...
| `S1` | `_Legacy_중립세력 (Backup)` | `legacy_root` | 삭제 검토가 아니라 reference-only 격리 상태를 검증한다 | 백업 흔적, 구형 naming, 비교 가치 확인 |
...
1. 루트 셋 `S1`을 먼저 확인한다
2. 그다음 범대륙 내부 대표 샘플 `S2` 두 개를 확인한다
3. 마지막에 에테르 control sample `S3`를 확인해
...
원본 접근과 실제 실측이 가능해질 때만
이 큐 순서를 reference sampling에 사용한다.

즉 `P0 root_corruption`의 watch 재개 조건은
막연한 추가 비교가 아니라,
