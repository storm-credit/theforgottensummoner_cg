# Section 15 People Worth Seeking Anchor Map
...
- `14` 신호가 강한 후보는 앵커만 기록하고 15 stable set 편입 판단은 하지 않는다.

## Anchor Table

| Name | Continent Anchor | Faction / Place Anchor | Role Tag | State Label | Routing Note |
|---|---|---|---|---|---|
| `실비아` | `범대륙 / 후기 확장` | `키르케 영약회` | `기록자 / 약제사` | `deferred_expansion_hold / hold reference split / name_collision_watch` | 키르케 내부 기억과 계량을 보여주는 `deferred_expansion_hold / hold reference split / name_collision_watch`이지만 범대륙 후기 확장축이라 stable triad package freeze와 분리 유지한다. |
| `멜리산드르` | `범대륙 / 후기 확장` | `키르케 영약회` | `수석 대약제사 / 조직 최고위` | `source_check_hold` | S+급 최고위라 14와 겹칠 수 있다. 14 확인 전 15 stable set에 넣지 않는다. |
| `울프가르 드래곤포지` | `크림슨` | `용의 후예 / 드래곤포지 공방` | `대장장이 / 공방주` | `named_notable_candidate / grade_caution / stable_15_workset / route_hierarchy_locked` | 드래곤포지 공방 앵커는 stable triad package freeze에 포함되지만 A급/전설 영웅록 신호가 있어 Hard Canon으로는 고정하지 않고 14 재확인 뒤로 둔다. |
| `드락사르 블레이즈포지` | `크림슨` | `용의 후예 / 연금술 공방` | `연금술사 / 공병대장` | `source_check_hold` | 공방 명사 가치가 있지만 A급 영웅표 신호가 있어 14 확인 전 source check만 유지한다. |
| `에리온 드라코비스` | `크림슨` | `엘드라칸 / 학술-전승층` | `대현자 / 고대어 해석가 / 관리자` | `named_notable_candidate / grade_caution / name_collision_watch / stable_15_workset / route_hierarchy_locked` | 엘드라칸 학술-전승층의 얼굴이며 stable triad package freeze에 포함되지만 A급/세력 핵심표 신호와 `에리온 베르날리스` 충돌 감시 때문에 caution을 유지한다. |
| `오그마` | `크림슨` | `엘드라칸 / 전승 보관층` | `고룡 / 살아있는 도서관 / 조언자` | `named_notable_candidate / stable_15_workset / act_watch / route_hierarchy_locked` | 전승 보관층 앵커는 stable triad package freeze에서 가장 안정적이지만 특정 Act 중심성이 급상승하면 14 경계를 재확인한다. |
...
| `아리안 블레이즈하트` | `크림슨` | `붉은 사막 부족 / 현자 회의` | `사제 / 전승 보존자` | `source_check_hold` | 부족 전승과 의례 축의 15 가치가 있지만 S급 대사제 신호가 있어 14 확인 전 15 stable set에 넣지 않는다. |
| `엘다라` | `에테르` | `정령연합 / 루미라` | `대현자 / 고대 정령어 권위자 / 의장` | `source_check_hold / hold reference split` | 에테르 정령연합의 언어/전승층을 보강하는 hold reference split이다. 다만 정령연합 전체 14 확인 전에는 `source_check_hold / hold reference split`로만 남긴다. |
| `크리스토퍼 델마르` | `해양` | `황금 함대 / 거상 연합` | `장물 금융 / 암시장 자금` | `source_check_hold` | 검은 동전, 장물 처리, 암시장 자금 담당 신호가 있으나 14/15 경계 확인 전 stable set 비편입으로 둔다. |

## Reading Snapshot

- `크림슨` 쪽 People Worth Seeking 후보가 가장 많다.
- `범대륙 / 후기 확장` 후보는 키르케 계열이지만, 범대륙 자체가 후순위라 `deferred_expansion_hold / hold reference split / name_collision_watch` reference로만 남긴다.
- `에테르` 쪽은 `엘다라`를 정령연합 내부 전승층의 hold reference split으로 보존하고, low-priority auxiliary slot 9개는 각 도시/기관 앵커 아래 role slot 유지로 closure를 마쳤다.
- `프로스트`, `오벨리스크`, `해양` 쪽은 안정 15 후보보다 장소-기관 역할 슬롯이 강하다. 특히 프로스트는 unnamed slot 6개가 모두 role slot 유지로 한 번 닫혔다.

## Policy Anchor Guard

- `크림슨` 앵커맵은 씨족 중심 질서와 학술/전승/공방 thin-support를 정리하는 용도지, `state_house strong` 근거 선언이 아니다.
...
- `실비아`는 `범대륙 후기 확장 / deferred_expansion_hold / hold reference split / name_collision_watch` 내부에서만 유지한다.
...
이 앵커맵은 그 closure 상태를 People Worth Seeking anchor 층에서만 참조한다.
...
| `프로스트` | tribe_clan/frontier survival 감각으로 읽고, 확정 인물보다 오로라 평원/빙하의 성소/아이스포지 슬롯을 보존한다. `아이스포지 병기소 장인`까지 named holder 없이 닫혀 unnamed slot 6개 closure가 한 번 완료됐다. |
| `해양` | state_house/guild_market spine이 본체. 포트 아우렐리온/크로스윈드 포트/오라클 바지 같은 도시 기능형 슬롯은 이미 closure-synced reference holding으로 읽는다. |
| `오벨리스크` | frontier_survival/guild_market spine이 본체. 기록/기억/봉인/망각 장소-기관 슬롯을 먼저 보존한다. |
| `범대륙 / 후기 확장` | 후순위 reference. 키르케 외 확장 세력은 표면명/위상 안정화 뒤 reference backlog로만 둔다. |
...
아래 결손층을 watch-reference로만 유지한다.

1. stable 15 triad의 `stable_15_workset` 상태어를 중앙 앵커맵까지 동일하게 유지
2. `엘다라`는 hold reference split 안의 `source_check_hold / hold reference split`로 유지하고, 정령연합 전체 14 확인 전에도 같은 결합 표기를 유지
3. `need_named_candidate` 슬롯의 장소-기관 색인은 계속 role slot 기준으로 유지
4. 14/15 경계 후보는 14 확인 전 stable set 비편입으로 유지
5. folder draft reference는 consistency lock 안에서만 유지하고, live 이동과 분리된 reference로 둔다.
6. continent sidecar/scout/display wording umbrella는
   lower current-state watch/reference authority로만 유지한다.

정찰 결과 reference는 `Section_15_Five_Continent_Closure_Table.md`와
`Section_8_to_15_Notable_Anchor_Bridge.md`를 우선 기준으로 삼는다.

## Conductor Rule

People Worth Seeking은 `대륙 -> 세력 / 도시 / 조직` 앵커로 보관한다.
