...
|---|---|---|---|---|---|
| `실비아` | `범대륙 / 후기 확장` | `키르케 영약회` | `기록자 / 약제사` | `deferred_expansion_hold / name_collision_watch` | 키르케 내부 기억과 계량을 보여주는 hold reference split이지만 범대륙 후기 확장축이라 `stable_triad_frozen_reference_set`과 분리 유지한다. |
...
| `엘다라` | `에테르` | `정령연합 / 루미라` | `대현자 / 고대 정령어 권위자 / 의장` | `source_check_hold / hold reference split` | 에테르 정령연합의 언어/전승층을 보강하는 hold reference split이다. 다만 정령연합 전체 14 확인 전에는 `source_check_hold`로만 남긴다. |
...
2. `엘다라`는 hold reference split 안의 `source_check_hold`로 유지하고, 정령연합 전체 14 확인 전 `source_check_hold`를 유지
