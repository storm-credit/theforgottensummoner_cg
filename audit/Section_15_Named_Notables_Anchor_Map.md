...
|---|---|---|---|---|---|
| `실비아` | `범대륙 / 후기 확장` | `키르케 영약회` | `기록자 / 약제사` | `deferred_expansion_hold / name_collision_watch` | 키르케 내부 기억과 계량을 보여주는 hold 후보지만 범대륙 후기 확장축이라 triad package와 분리 유지한다. |
| `멜리산드르` | `범대륙 / 후기 확장` | `키르케 영약회` | `수석 대약제사 / 조직 최고위` | `verify_before_15` | S+급 최고위라 14와 겹칠 수 있다. 15 확정 금지. |
| `울프가르 드래곤포지` | `크림슨` | `용의 후예 / 드래곤포지 공방` | `대장장이 / 공방주` | `named_notable_candidate / grade_caution / stable_15_workset / route_hierarchy_locked` | 드래곤포지 공방 앵커는 triad package freeze reference에 포함되지만 A급/전설 영웅록 신호가 있어 Hard Canon 고정은 14 재확인 뒤로 둔다. |
| `드락사르 블레이즈포지` | `크림슨` | `용의 후예 / 연금술 공방` | `연금술사 / 공병대장` | `verify_before_15` | 공방 명사 가치가 있지만 A급 영웅표 신호가 있어 14 확인 필요. |
| `에리온 드라코비스` | `크림슨` | `엘드라칸 / 학술-전승층` | `대현자 / 고대어 해석가 / 관리자` | `named_notable_candidate / grade_caution / name_collision_watch / stable_15_workset / route_hierarchy_locked` | 엘드라칸 학술-전승층의 얼굴이며 triad package freeze에 포함되지만 A급/핵심표 신호와 `에리온 베르날리스` 충돌 감시 때문에 caution을 유지한다. |
| `오그마` | `크림슨` | `엘드라칸 / 전승 보관층` | `고룡 / 살아있는 도서관 / 조언자` | `named_notable_candidate / stable_15_workset / act_watch / route_hierarchy_locked` | 전승 보관층 앵커는 triad package freeze reference에서 가장 안정적이지만 특정 Act 중심성이 급상승하면 14 경계를 재확인한다. |
...
| `엘다라` | `에테르` | `정령연합 / 루미라` | `대현자 / 고대 정령어 권위자 / 의장` | `named_notable_candidate / source_check_hold / support_hold` | 에테르 정령연합의 언어/전승층을 보강하는 `support_hold`다. 다만 정령연합 전체 14 확인 전 Hard Canon 고정은 보류한다. |
...
1. stable 15 triad의 package freeze 문구를 중앙 앵커맵까지 동일하게 유지
2. `엘다라`는 `support_hold`로 유지하고, 정령연합 전체 14 확인 전 Hard Canon 고정 금지
3. `need_named_candidate` 슬롯의 장소-기관 색인은 계속 role slot 기준으로 유지
4. 14/15 경계 후보는 14 확인 전 확정 금지
5. folder draft reference는 consistency lock 안에서만 유지하고, live 이동 판단과 분리한다.
