...
| `울프가르 드래곤포지` | `크림슨` | `용의 후예 / 드래곤포지 공방` | `대장장이 / 공방주` | `named_notable_candidate / grade_caution / stable_15_workset` | 드래곤포지 공방 앵커는 안정적이지만 A급/전설 영웅록 신호가 있어 Hard Canon 승격은 14 재확인 뒤로 미룬다. |
| `드락사르 블레이즈포지` | `크림슨` | `용의 후예 / 연금술 공방` | `연금술사 / 공병대장` | `verify_before_15` | 공방 명사 가치가 있지만 A급 영웅표 신호가 있어 14 확인 필요. |
| `에리온 드라코비스` | `크림슨` | `엘드라칸 / 학술-전승층` | `대현자 / 고대어 해석가 / 관리자` | `named_notable_candidate / grade_caution / name_collision_watch / stable_15_workset` | 엘드라칸 학술-전승층의 얼굴이지만 A급/핵심표 신호와 `에리온 베르날리스` 충돌 감시 때문에 caution을 유지한다. |
| `오그마` | `크림슨` | `엘드라칸 / 전승 보관층` | `고룡 / 살아있는 도서관 / 조언자` | `named_notable_candidate / stable_15_workset / act_watch` | 전승 보관층 앵커는 가장 안정적이지만 특정 Act 중심성이 급상승하면 14 경계를 다시 본다. |
...
| `엘다라` | `에테르` | `정령연합 / 루미라` | `대현자 / 고대 정령어 권위자 / 의장` | `named_notable_candidate / verify_source_before_profile` | 에테르 정령연합의 언어/전승층을 보강하는 가장 강한 보조 후보다. 다만 정령연합 전체 14 확인 전 Hard Canon 승격은 보류한다. |
...
1. stable 15 triad의 hardening / route freeze 문구를 중앙 앵커맵까지 동일하게 유지
2. `엘다라`는 보조 후보로 유지하고, 정령연합 전체 14 확인 전 Hard Canon 승격 금지
3. `need_named_candidate` 슬롯의 장소-기관 색인은 계속 role slot 기준으로 유지
4. 14/15 경계 후보는 14 확인 전 확정 금지
5. 실제 폴더 초안은 consistency lock 이후 commit/push 판단이 끝난 뒤에만 다시 검토
