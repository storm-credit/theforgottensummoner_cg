# Section 15 Group Draft Mercenary Operations

이 문서는 `자유도시 용병 연맹` 실무층을
`15번`에서 어떻게 회수할지 정리한 조직별 초안이다.

## Group Identity

- display reading:
  - `자유도시 용병 연맹 실무층`
- core function:
  - 계약 보증
  - 분쟁 판정
  - 배신자 추적
  - 도시 지부 운영

이 묶음은 용병 연맹을
단순 전투 조직이 아니라 `계약 질서를 가진 사회`로 보이게 한다.

## Core Members

### 1. 용병 길드 마스터 계열

- role:
  - 도시 지부장
  - 계약망 총괄
- suggested tag:
  - `mercenary_guild_master`
- primary space:
  - `길드 본부`
- archive use:
  - 도시별 용병 질서와 지역 액트 연결

### 2. 용병 중재 위원회 심판관 계열

- role:
  - 계약 분쟁 판정자
  - 배신 심리 담당
- suggested tag:
  - `contract_arbiter`
- primary space:
  - `중재 법정`
- archive use:
  - 용병 사회의 규칙과 균열 복구

### 3. 블랙리스트 기록관 계열

- role:
  - 추방자 명부 관리자
  - 악질 의뢰인 기록관
- suggested tag:
  - `blacklist_warden`
- primary space:
  - `추방자 기록실`
- archive use:
  - 배신과 추적, 재등장 떡밥 축 보강

## Suggested Sub-Buckets

- `계약 운영층`
  - 용병 길드 마스터 계열
- `판정 질서층`
  - 용병 중재 위원회 심판관 계열
- `기록 및 추방층`
  - 블랙리스트 기록관 계열

## Policy Group Guard

- 이 묶음은 자유도시 `urban_market` 내부의 `contract order / guild discipline` 운영층을 보강하는 그룹이다.
- 계약 보증, 분쟁 판정, 블랙리스트 기록은 도시 용병 사회의 실무 질서이지,
  정규 국가군 지휘 체계나 전통 귀족국가 상층 복원 근거가 아니다.
- 즉 `자유도시 용병 연맹` 실무층은 시장-계약 사회의 자치 장치로만 읽고,
  왕국형 군제나 혈통 기사단 본체로 과독해하지 않는다.
- 이 group draft는 family-level carryover만 요약하고,
  exact guard wording authority는 연결되는 `Section_15_Profile_*` 카드의 `3-1. Policy Guard`에 남긴다.
- 따라서 이 문서는 downstream profile guard text를 다시 써서 대체하지 않는다.
- exact operational guard wording authority는 각 연결 profile 카드의 `3-1. Policy Guard`에 남고,
  이 group draft는 그 wording source를 mercenary-operations family 층에서만 요약한다.
- subline 확장까지 내려간 경우에도
  exact operational guard wording authority는 각 downstream `Section_15_Subline_Profile_*` 카드의
  `3-1. Policy Guard`에 남고,
  이 group draft는 그 wording source를 mercenary-operations family 층에서만 요약한다.

## Relation Hooks

- `다리안 벨가르드`
  - 영웅적 계약 보증과 실무 계약 질서의 연결
- `마르코 라메즈`
  - 합법 용병 질서와 불법 전장 소모 질서의 대비
- `세실리아 메르카토르`
  - 도시 질서 유지 차원에서 공적 접점이 생길 수 있다.
- `용병 의뢰인 / 배신자 / 악질 상단`
  - 계약 사회의 실제 갈등 대상들

## Spatial Hooks

- `머시너리 게이트`
- `길드 본부`
- `중재 법정`
- `추방자 기록실`
- `계약 공고 게시벽`

## Conductor Use

이 묶음은
자유도시의 용병 사회가 `싸움`보다 `계약`으로 굴러간다는 감각을 살리는 데 유용하다.

다만 `People Worth Seeking` 승인 논리와는 분리하고,
operational lower-card carryover reference로만 유지한다.
