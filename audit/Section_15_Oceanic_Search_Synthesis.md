...
이번 `top 5 slot` read-only pass에서는
`신탁 방주`, `해로 장부관`, `왕실 선공장 수석장`, `흑조 감정관`의 direct holder가 끝내 보이지 않았고,
`심연 장부관`은 `이소벨 골드리프`라는 상위 재무 권력축만 인접하는 것으로 정리됐다.

이번 `city-role batch`에서는
...
- `은행장`은 무역/보험/전쟁 펀딩 문맥이 반복되지만 named holder는 여전히 없다.
...
해양 unnamed slot read-only pass는 1차 마감한다.

다음 배치는 `오벨리스크`와 `프로스트` 중
다음 메인 본선 우선순위를 재고정하는 것이다.
