...
- `working/imports/phase4_section8_oceanic_golden_armada/8. 경제 및 상업 (Economy & Commerce)/1. 황금 함대 주요 교역로 및 무역 거점.md`
...
| `수석 기상관` | `크로스윈드 포트 (Crosswind Port)` | `수석 기상관`이 `바람의 탑 주인`, `정확한 기상 예보 및 출항권 획득` 역할로 적힌다. | `role_slot_confirmed` | 기상/출항 통제 슬롯은 강하지만 direct holder는 미확인이다. |
| `대경매장 주인` | `포트 아우렐리온 (Port Aurelion)` | `대경매장 주인`이 `모든 물건의 가치를 꿰뚫어 보는 감정사`, `정보상 역할 겸임`으로 직접 적힌다. | `role_slot_confirmed` | 경매/감정 슬롯은 강하지만 개인명은 없다. `크리스토퍼 델마르`와 병합하지 않는다. |
...
2. `스톰 체이서 대장`
3. `조선공 길드 장인 / 검은돛 선장인`
4. `진혼의 합창단 악기 보관인 / 진혼 악기지기`
5. `죽은 선장의 항해일지 보관인 / 망자항해 기록관`
