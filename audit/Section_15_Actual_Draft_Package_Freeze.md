# Section 15 `stable_triad_frozen_reference_set`
...
- live 파일명 변경도 하지 않는다.
- 이 문서는 `cg` 안의 `stable_triad_frozen_reference_set` 기준서다.
- freeze 대상은 stable triad만이다.
- `엘다라`, `실비아`는 hold reference split으로 분리한다.

## Freeze Scope

현재 `stable_triad_frozen_reference_set`은 아래 문서군을 한 묶음으로 본다.
...
8. 자유도시/오벨리스크 operational profile cluster는 stable_triad_frozen_reference_set 바깥에서만 유지하고, 각각 `urban_market` 또는 `nontraditional elite thin-support / dark institution` 범위를 넘겨 읽지 않는다.
...
현재 `stable_triad_frozen_reference_set`은 문서 레벨 reference로만 유지 가능하다.

현재 `Section 15` 내부 기준은
`stable_triad_frozen_reference_set`을 닫힌 상태로 유지하고,
hold reference split과 carryover wording consistency를 유지하는 것이다.

즉 이 문서는
`stable_triad_frozen_reference_set` 재개 문서가 아니라,
