# Section 15 Stable Triad Frozen Reference Set

이 문서는 stable triad actual draft 묶음을
`여기까지를 frozen reference set 범위로 본다`는 기준으로 정리한 frozen reference 시트다.

중요:

- live 폴더 생성/이동은 하지 않는다.
- live 파일명 변경도 하지 않는다.
- 이 문서는 `cg` 안의 stable triad frozen reference set 기준서다.
- freeze 대상은 stable triad만이다.
- `엘다라`, `실비아`는 hold reference split으로 분리한다.

## Freeze Scope

현재 stable triad frozen reference set은 아래 문서군을 한 묶음으로 본다.
...
| `실비아` | `deferred_expansion_hold / name_collision_watch` | 범대륙 후기 확장축은 triad package와 분리된 deferred hold reference로만 유지한다. |
...
7. stable triad의 카드별 `Policy Guard`는 frozen reference set 아래에서도 유효하며, `state_house strong` 고정 근거로 넘겨 읽지 않는다.
...
11. frozen reference summary는 lower-card authority를 재정의하지 않고,
...
14. representative `Port Authority / Black Auction / Gravewell / Counterfeit Workshop` pair는
    ...
    frozen reference set은 그 closure 상태를 package watch 층에서만 참조한다.
15. `결손층 5개`의 thin/support 판정은 frozen reference set이 새로 결정하지 않고,
...
이 frozen reference set이 watch baseline으로 남아 있는지 확인하는 무결성 점검이다.
...
4. revision gate가 triad frozen reference set과 충돌하지 않는다.
...
현재 stable triad frozen reference set은 문서 레벨 reference로만 유지 가능하다.

현재 `Section 15` 내부 기준은
stable triad frozen reference set을 닫힌 상태로 유지하고,
hold reference split과 carryover wording consistency를 유지하는 것이다.
