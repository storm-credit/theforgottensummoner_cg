...
- closed reference/watch snapshot에서 위험을 먼저 확인한다.
...
| `G3. Boundary Safety` | `verify_before_15` 후보를 live 15 고정자로 다루지 않았는가? | `pass` |
...
| `G9. support_hold Separation` | `엘다라`가 stable triad frozen reference set에 섞이지 않고 `support_hold`로 분리되어 있는가? | `pass_with_support_hold` |
...
| `G12. Package Freeze Completeness` | stable triad frozen reference set과 hold reference split이 별도 freeze 시트까지 포함해 잠겨 있는가? | `pass_with_frozen_reference_lock` |
...
| `범대륙 후기 확장 과확정` | `medium` | 키르케는 강한 후보만 보존하고 전체 범대륙은 mainline reference 바깥 hold로 유지. |
...
stable triad frozen reference set과 carryover watch용으로만 유지 가능하다.

하지만 live 폴더 이동 또는 문서 이동은 아직 금지한다.

현재는 아래 두 가지를 `reference/watch 기준`으로만 유지한다.

1. stable triad frozen reference set 유지와 closure sync / carryover watch 점검
...
closed reference/watch snapshot을 유지한 채,
