...
| `G8. Stable Triad Route Safety` | `울프가르 / 에리온 / 오그마`가 `드래곤포지 공방 / 학술-전승층 / 전승 보관층` reference route set 기준으로 분리되어 있는가? | `pass_with_actual_draft_lock` |
...
stable triad actual draft reference와 carryover watch용으로만 유지 가능하다.

하지만 live 폴더 이동 또는 문서 이동은 아직 금지한다.

현재는 아래 두 가지를 `reference/watch 기준`으로만 유지한다.

1. stable triad actual draft reference split 유지와 closure sync / carryover watch 점검
