...
| Area | `stable_triad_frozen_reference_set` | Source-Check Hold | Main Risk | Reference Action |
|---|---|---|---|---|
...
| `에테르` | 없음 | `엘다라 [source_check_hold]`, `대런`, `엘드린`, `마르쿠스/맥스웰`, `이사도르`, `세리오스`, `네리사`, `다미엔`, `칼리스트`, `래퍼티`, `요한`, `엘라라`, `드라이덴`, `메라`, `실라스 나이트쉐이드` | 탑주, A/S급, Act 신호, 이름 드리프트, 동명이인 충돌 | `Ravenfell`, `Mera`, `Sylas` 병합 금지. 에테르 surface-name baseline과 spirit-union hold reference split closure가 모두 잠긴 watch-reference 상태다. 새 hold 확장보다 `5대륙 closure sync / watch-reference` 유지가 우선이다. |
...
- `엘다라`는 hold reference split 안의 `source_check_hold`로만 유지한다.
