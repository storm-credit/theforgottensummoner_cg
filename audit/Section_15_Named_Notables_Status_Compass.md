...
| `범대륙 / 후기 확장` | `실비아 [deferred_expansion_hold]` | `멜리산드르` | 후기 증설 구역, 키르케 위상 과확정 위험 | mainline reference 바깥 hold다. 이름 톤과 앵커 안정화 뒤 재검토한다. |
...
- `엘다라`는 에테르 `support_hold`이며, 큐 9에서 루미라 대현자/고대 정령어 권위자 support basis 메모가 잠겨 있다. 단, 정령연합 전체 14 확인 전 Hard Canon으로 고정하지 않는다.
- `실비아`는 키르케 계열 `deferred_expansion_hold`이며 범대륙 후기 확장 구역이라 `deferred_expansion_hold` reference로만 둔다.
