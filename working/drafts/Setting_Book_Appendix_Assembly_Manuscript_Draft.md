...
## Setting Book Appendix Assembly Manuscript Draft

## Note

이 문서는 appendix 샘플들을 실제 부록처럼 묶어 본 조립 초안이다.
...
`이 세계를 읽는 법`과 `다섯 대륙`은 별도의 새 부록으로 분리하지 않는다.
...
4. 왕국, 성국, 자유도시, 마법협회, 정령연합의 긴장은 `다섯 대륙` 본문에서는 감각으로 보여주고, 구조 확정은 Chapter 1과 Appendix A/C/E로 되돌려 확인한다.

따라서 v1 전까지는 `이 세계를 읽는 법` / `다섯 대륙` 전용 Appendix F를 만들지 않는다.
...
- 물건인지, 물질인지, 기술인지, 인명 충돌인지 먼저 가른다.
- 유물은 성능보다 욕망과 비용으로 먼저 설명한다.
...
- 장소는 배경이 아니라 기능이다.
- 길은 거리보다 비용과 통제의 문제다.
- 같은 장소도 어떤 경로로 들어오느냐에 따라 전혀 다른 장면이 된다.
...
- 대륙, 세력, 성, 이명, 역할이 다르면 우선 분리한다.
- 인물명과 유물명, 마법명, 개념명 충돌은 별도로 본다.
...
본문은 세계가 어떻게 느껴지는지 말하고,
부록은 왜 그렇게 말해도 되는지 보여준다.

따라서:

- 본문에 너무 무거운 표는 부록으로 보낸다.
- 부록에서 새 lore를 발명하지 않는다.
- 부록의 후보 표는 본문 확정 엔트리로 자동 승격되지 않는다.
- 부록은 통제 장치이면서도, 다음 확장을 위한 기억 장치여야 한다.
