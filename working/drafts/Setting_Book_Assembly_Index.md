...
1. Add row-level evidence notes to Appendix B/C only if a specific item or place row needs verification.
2. Re-evaluate the release-candidate file trigger only after v1 gates tighten.
...
The next meaningful work is controlled refinement: keep `preview_v0_readable` stable as the shareable package, keep Appendix A/D/E row-level notes as the targeted evidence pass, add Appendix B/C row notes only on demand, and re-open the release-candidate file question only when packaging becomes the next preserved artifact.
